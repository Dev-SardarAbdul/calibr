...
import { adminNewProductsTableData } from "../../../data";
      ...
      <div className="w-full mx-auto lg:w-3/4">
        <div className="flex flex-col items-center justify-between gap-4">
          <h2 className="text-center text-[32px]  font-[600] secondary-font text-secondary">
            Update Home New Products Section Content
          </h2>
          <button
          ...
          <div className="inline-flex w-full overflow-x-auto border border-slate-300">
            <div className="min-w-[650px] w-full bg-white max-h-[500px] overflow-auto">
              <div className="flex items-center justify-between w-full gap-8 p-4">
                {adminNewProductsTableData.map((item) => (
        ...
        <AnimatePresence>
          {showAddModal && (
            <AddProductModal setShowAddModal={setShowAddModal} />
          )}
        </AnimatePresence>
        <AnimatePresence>
          {showEditModal && (
            <EditProductModal setShowEditModal={setShowEditModal} />
          )}
        </AnimatePresence>
        <AnimatePresence>
          {showDeleteModal && (
            <DeletProductModal setShowDeleteModal={setShowDeleteModal} />
          )}
        </AnimatePresence>
      </div>
    </div>
